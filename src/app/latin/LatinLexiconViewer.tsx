...
// const BASE_URL = 'http://localhost:3001/api/latin'; // For local dev
...
const LatinLexiconViewer: React.FC<LatinLexiconViewerProps> = ({
  ...
  useEffect(() => {
    if (latinWord && latinWord !== wordToFetch) {
      setWordToFetch(latinWord);
    }
  }, [latinWord, wordToFetch]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const lexiconUrl = `${BASE_URL}/lexica/${encodeURIComponent(wordToFetch.trim())}`;
        ...
        const responseData: LatinLexiconResponse = await lexiconRes.json();
  ...
  }, [wordToFetch, showLexicalModal, historyNamespace]);
